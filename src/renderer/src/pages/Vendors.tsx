  ...
  return (
    <div className="flex flex-col h-full">
      <div className="shrink-0 border-b bg-white px-4 py-3 flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-charcoal-800">Vendor Directory</span>
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="grid gap-3">
          {vendors.map((v, i) => (
            <button
              key={v.name}
              className="w-full text-left rounded-lg border border-silver-200 bg-white hover:border-brand/40 hover:shadow-sm transition-all group"
            ...
            >
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-charcoal-800 truncate">{v.name}</p>
                ...
                <div className="shrink-0 text-right hidden sm:block">
                  <p className="text-xs font-mono font-semibold text-charcoal-800">{v.onHand.toLocaleString()}</p>
                  <p className="text-[10px] text-muted-foreground">on hand</p>
                </div>
                <div className="shrink-0 text-right hidden md:block ml-4">
                  <p className="text-xs font-mono font-semibold text-charcoal-700 flex items-center gap-0.5">
              ...
              <>
                ...
                <div className="flex gap-4 mt-2">
                  <div>
                    ...
                    <p className="text-lg font-bold text-charcoal-800">{selectedVendor.skus}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">On Hand</p>
                    <p className="text-lg font-bold text-charcoal-800">{selectedVendor.onHand.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Sales/Wk</p>
                    <p className="text-lg font-bold text-charcoal-800">{selectedVendor.salesPerWeek.toLocaleString()}</p>
                  </div>
                </div>
                {selectedVendor.categories.length > 0 && (
                  <SheetDescription className="flex flex-wrap gap-1 mt-1">
                    {selectedVendor.categories.map(c => (
                      <span key={c} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-blue-50 text-brand border border-blue-100">{c}</span>
                    ))}
                  </SheetDescription>
                )}
              </>
                  ...
                  {selectedVendor.items.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2">
                        <p className="font-medium text-charcoal-800 leading-tight">{item.description}</p>
                        <p className="text-[10px] font-mono text-muted-foreground">{item.itemCode}</p>
                      </td>
                      <td className="px-3 py-2 text-right font-mono">
                        <span className={cn('font-semibold', item.onHand < 0 && 'text-destructive')}>
